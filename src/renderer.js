...
const DIFF_PANEL_MAX_WIDTH_RATIO = 0.7; // max 70% of window
...
function toggleDiffPanel() {
  ...
  // Close status panel if opening diff panel
...
function renderDiffContent(file) {
  ...
  if (diffViewMode === 'unified') {
    ...
    // Split view: pair deletions with additions
    const leftLines = [];
    const rightLines = [];
    let i = 0;
    ...
    const renderSide = (lines, isLeft) => lines.map(l => {
      if (l.type === 'hunk') return `<div class="diff-hunk-header">${escapeHtml(l.text)}</div>`;
      const cls = l.type === 'add' ? 'add' : l.type === 'del' ? 'del' : l.type === 'empty' ? 'ctx' : 'ctx';
...
// Show accent bar when hovering the resize edge
