...
describe('UpdateService', () => {
  ...
  describe('missing release artifact — treated as not-available', () => {
    function freshService() {
      const mainWindow = makeMainWindow();
      const svc = new UpdateService(mainWindow, makeSettingsService(), { autoUpdater: mockAutoUpdater, ipcMain: mockIpc.ipcMain });
      return { svc, mainWindow };
    }

    it('maps a 404 error event to not-available with current version', () => {
      const { svc, mainWindow } = freshService();
      mockAutoUpdater.emit('error', new Error('HttpError: 404 Not Found'));
      expect(svc.status).toBe('not-available');
      expect(svc.error).toBeNull();
      expect(svc.updateInfo).toEqual({ version: '1.2.0' });
    ...
    it('maps "Cannot find latest-mac.yml" to not-available', () => {
      const { svc } = freshService();
      mockAutoUpdater.emit('error', new Error('Cannot find latest-mac.yml in the latest release artifacts'));
      expect(svc.status).toBe('not-available');
      expect(svc.error).toBeNull();
    });

    it('maps statusCode 404 (no message match) to not-available', () => {
      const { svc } = freshService();
      const err = new Error('Some other text');
      err.statusCode = 404;
      mockAutoUpdater.emit('error', err);
      expect(svc.status).toBe('not-available');
    });

    it('still reports real errors as error', () => {
      const { svc } = freshService();
      mockAutoUpdater.emit('error', new Error('ENOTFOUND github.com'));
      expect(svc.status).toBe('error');
      expect(svc.error).toMatch(/ENOTFOUND/);
    });

    it('update:check IPC returns not-available when artifact is missing', async () => {
      const { svc } = freshService();
      mockAutoUpdater.checkForUpdates.mockRejectedValueOnce(new Error('HttpError: 404'));
      const result = await mockIpc.handlers['update:check']();
      expect(svc.status).toBe('not-available');
      expect(result).toEqual({ status: 'not-available', info: { version: '1.2.0' } });
    });

    it('update:check IPC propagates real errors', async () => {
      const { svc } = freshService();
      mockAutoUpdater.checkForUpdates.mockRejectedValueOnce(new Error('connect ECONNREFUSED'));
      const result = await mockIpc.handlers['update:check']();
      expect(svc.status).toBe('error');
